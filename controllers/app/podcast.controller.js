...
exports.createPodcast = async (req, res) => {
  try {
    const { name, parent, image, episodes, url, genre, subgenre, season, tags, timestamp } = req.body;
...
// PATCH /api/podcasts/:id/toggle
exports.togglePodcastStatus2 = async (req, res) => {
  try {
    const { id } = req.params;
    const { isActive } = req.body;
...
exports.deletePodcast = async (req, res) => {
  try {
    ...
    // Check if this podcast is a parent to any other podcast
    const child = await Podcast.findOne({ parent: id });
    if (child) {
      return res.status(400).json({
        message: 'Cannot delete podcast because it has children. Please delete its children first.',
